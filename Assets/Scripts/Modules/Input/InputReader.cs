...
namespace Metroidvania.InputSystem
{
    ...
    public class InputReader : ScriptableSingleton<InputReader>, InputActions.IGameplayActions, InputActions.IMenusActions
    {
        ...
        public event Action DashEvent;        public event Action<float> DirectionalDashEvent;
        ...
        void InputActions.IGameplayActions.OnHeavyAttack(InputAction.CallbackContext context)
        {
            if (context.phase == InputActionPhase.Performed)
                HeavyAttackEvent?.Invoke();
        }
        
        void InputActions.IGameplayActions.OnDirectionalDash(InputAction.CallbackContext context)
        {
            if (context.phase == InputActionPhase.Performed)
            {
                UnityEngine.Debug.Log($"DirectionalDash triggered!");
                
                // Get the path of the control that was pressed
                string controlPath = context.control.path;
                UnityEngine.Debug.Log($"Control path: {controlPath}");
                
                float direction = 1f; // default right
                
                // Check if the control path contains leftArrow or rightArrow
                if (controlPath.Contains("leftArrow"))
                {
                    direction = -1f;
                    UnityEngine.Debug.Log("Dash LEFT detected!");
                }
                else if (controlPath.Contains("rightArrow"))
                {
                    direction = 1f;
                    UnityEngine.Debug.Log("Dash RIGHT detected!");
                }
                
                DirectionalDashEvent?.Invoke(direction);
            }
        }
